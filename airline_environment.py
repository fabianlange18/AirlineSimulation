...
from util.consumer_behavior import consumer_demand_timewise, consumer_demand_price_wise
...
class AirlineEnvironment(gym.Env):
    ...
    def get_p_dist(self, a, timestep):
        x_t = (10 / self.booking_time) * (timestep + 1)
        x_p = (10 / self.booking_time) * timestep

        price_trend = (1 - (self.transform_action(a) * consumer_demand_price_wise(x_p)) / self.max_price)
        time_sensitivity = 1.7
        time_trend = consumer_demand_timewise(x_t) * time_sensitivity

        p = price_trend * time_trend
